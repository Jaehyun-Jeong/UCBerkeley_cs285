...
class PGAgent(nn.Module):
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            values = self.critic(torch.Tensor(obs).to(ptu.device))
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        return_value = sum([(self.gamma**idx)*reward for idx, reward in enumerate(rewards)])

        return np.array([return_value for _ in rewards])
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for t in range(len(rewards)):
            reward_to_go_lst.append(
                sum([(self.gamma**idx)*reward for idx, reward in enumerate(rewards[t:])])
            )

        reward_to_go_lst = np.array(reward_to_go_lst)

        return reward_to_go_lst
