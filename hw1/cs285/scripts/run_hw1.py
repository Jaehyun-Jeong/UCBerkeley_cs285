    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            if params['do_dagger']:
                ...
                actor_observation = torch.Tensor(paths[0]['observation']).to(ptu.device)

                # and replace paths[i]["action"] with these expert labels
                paths[0]['action'] = expert_policy.forward(actor_observation).data.cpu().numpy()
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          batch_idx = np.random.permutation(len(replay_buffer)) #[:param['train_batch_size']]
